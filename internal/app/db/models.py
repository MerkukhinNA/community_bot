...
from db.enum import VisitStatus, FeedbackStatus
...
class Event(Base):
    ...
    visits: Mapped[list['Visit']] = relationship('Visit', back_populates='event') 
...
class Visit(Base):
    ...
    status: Mapped[VisitStatus] = mapped_column(Enum(VisitStatus), nullable=False, default=VisitStatus.CREATED)
    deleted: Mapped[bool] = mapped_column(Boolean(), nullable=True, default=False)

    event: Mapped['Event'] = relationship('Event', back_populates='visits', lazy='joined') 
    user: Mapped['User'] = relationship('User', back_populates='visits', lazy='joined') 
    

class Feedback(Base):
    __tablename__ = "feedbacks"
        
    feedback_id: Mapped[int] = mapped_column(Identity(), primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(Integer(), ForeignKey('users.user_id'), nullable=True)
    text: Mapped[str] = mapped_column(Text(), nullable=True)
    status: Mapped[FeedbackStatus] = mapped_column(Enum(FeedbackStatus), nullable=False, default=FeedbackStatus.CREATED)
    date: Mapped[dt] = mapped_column(TIMESTAMP(), nullable=True)
    deleted: Mapped[bool] = mapped_column(Boolean(), nullable=True, default=False)

    user: Mapped['User'] = relationship('User', back_populates='feedbacks', lazy='joined')
    answer: Mapped['Answer'] = relationship('Answer', back_populates='feedback', lazy='joined') 
...
class Answer(Base):
    ...
    user_id: Mapped[int] = mapped_column(Integer(), ForeignKey('users.user_id'), nullable=True)
    feedback_id: Mapped[int] = mapped_column(Integer(), ForeignKey('feedbacks.feedback_id'), nullable=True)
    text: Mapped[str] = mapped_column(Text(), nullable=True)
    date: Mapped[dt] = mapped_column(TIMESTAMP(), nullable=True)
    deleted: Mapped[bool] = mapped_column(Boolean(), nullable=True, default=False)

    user: Mapped['User'] = relationship('User', back_populates='answers', lazy='joined')
    feedback: Mapped['Feedback'] = relationship('Feedback', back_populates='answer', lazy='joined')